...
export default function Reports({ userId, expenses }: ReportsProps) {
  ...
  const fetchReportData = async () => {
    ...
    setExportError(null);
    try {
      const response = await fetch(getApiUrl(`/api/reports/monthly?month=${selectedMonth}&year=${selectedYear}`), {
  ...
  const handleExport = async (format: 'pdf' | 'excel') => {
    ...
    setExportError(null);
    try {
      const response = await fetch(getApiUrl(`/api/reports/export/${format}?month=${selectedMonth}&year=${selectedYear}`), {
        headers: { 'x-user-id': userId.toString() }
      });
      if (response.ok) {
        ...
        throw new Error(text || `HTTP ${response.status} met on generating report`);
      }
    } catch (error: any) {
  ...
  // Generate a short monthly summary note
  const getSummaryNote = () => {
    if (!reportData) return "";
    const overBudget = reportData.budgetStatus.filter(b => b.percentage >= 100);
    const savingsNote = reportData.totalSpent < 5000 ? "Excellent expense control this period." : "Spending aligned with historical averages.";
    
    if (overBudget.length > 0) {
      return `${overBudget[0].category} exceeded budget by ${formatAmount(overBudget[0].spent - overBudget[0].limit)} this month. ${savingsNote}`;
    }
    ...
    <div className="mt-8 space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 p-8 bg-card rounded-[2.5rem] border border-border shadow-soft">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-black text-text-main dark:text-white tracking-tight">Financial Reports</h2>
            <p className="text-[10px] font-bold text-text-muted uppercase tracking-[0.3em] opacity-60">Audit and Export Center</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {loading ? (
            ...
            >
              ...
              <div className="bg-card p-12 rounded-[2.5rem] border border-border shadow-polish relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-16">
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-[10px] font-black text-primary uppercase tracking-[0.4em]">Official Statement</span>
                    </div>
                    <h3 className="text-3xl font-black text-text-main dark:text-white tracking-tight leading-none mb-2">Monthly Ledger</h3>
                    <p className="text-xs font-bold text-text-muted uppercase tracking-[0.2em] opacity-60">{MONTHS[selectedMonth]} {selectedYear}</p>
                  </div>
                  <div className="text-right p-6 bg-bg dark:bg-slate-950/20 rounded-3xl border border-border/50">
                    <p className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em] mb-2 opacity-60">Statement Total</p>
                    <p className="text-4xl font-black text-text-main dark:text-white leading-none tracking-tight">{formatAmount(reportData.totalSpent)}</p>
                  </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-3 gap-12 relative z-10 mb-12">
                  <div className="space-y-2">
                    <p className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em] leading-none opacity-60">Reconciled Items</p>
                    <p className="text-2xl font-black text-text-main dark:text-white tracking-tight">{reportData.transactionCount}</p>
                  </div>
                  <div className="space-y-2">
                    <p className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em] leading-none opacity-60">Peak Classification</p>
                    <p className="text-base font-black text-text-main dark:text-white truncate tracking-tight">{reportData.highestCategory?.name || 'N/A'}</p>
                    <p className="text-[10px] font-bold text-primary italic uppercase tracking-widest">{reportData.highestCategory ? formatAmount(reportData.highestCategory.amount) : '-'}</p>
                  </div>
                  <div className="space-y-2">
                    <p className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em] leading-none opacity-60">Base Classification</p>
                    <p className="text-base font-black text-text-main dark:text-white truncate tracking-tight">{reportData.lowestCategory?.name || 'N/A'}</p>
                    <p className="text-[10px] font-bold text-text-muted italic uppercase tracking-widest">{reportData.lowestCategory ? formatAmount(reportData.lowestCategory.amount) : '-'}</p>
                  </div>
                </div>

                {/* Category Audit List (New Section for Reports) */}
                <div className="relative z-10 mt-12 pt-12 border-t border-border/50">
                  <h4 className="text-[10px] font-black text-text-muted uppercase tracking-[0.3em] mb-6">Expenditure Audit Table</h4>
                  <div className="space-y-4">
                    {Object.entries(reportData.categoryBreakdown).map(([cat, amt]) => (
                      <div key={cat} className="flex items-center justify-between py-3 border-b border-border/20 last:border-0 group">
                        <div className="flex items-center gap-3">
                          <div className="w-1.5 h-1.5 rounded-full bg-primary opacity-40 group-hover:opacity-100 transition-opacity" />
                          <span className="text-[13px] font-bold text-text-main dark:text-gray-300">{cat}</span>
                        </div>
                        <span className="text-[13px] font-black text-text-main dark:text-white">{formatAmount(amt)}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="mt-12 p-8 bg-primary/5 rounded-[2rem] border border-primary/10 flex items-start gap-6">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-2">Executive Summary</p>
              ...
              <div className="bg-card p-12 rounded-[2.5rem] border border-border shadow-polish">
                <div className="flex items-center justify-between mb-12">
                  <div>
                    <h3 className="text-xl font-black text-text-main dark:text-white uppercase tracking-tight leading-none mb-1">Budget Reconciliation</h3>
                    <p className="text-xs font-bold text-text-muted italic opacity-60">Status of projected allocation vs actual variance</p>
                  </div>
                ...
                <div className="space-y-6">
                  {reportData.budgetStatus.length > 0 ? (
                      ...
                      <table className="w-full text-left">
                        <thead>
                          <tr className="bg-bg dark:bg-slate-900 border-b border-border/50">
                            ...
                            <th className="px-6 py-4 text-[10px] font-black text-text-muted uppercase tracking-widest text-right">Variance</th>
                          </tr>
                        </thead>
                            ...
                            return (
                              <tr key={budget.category} className="group hover:bg-bg/50 transition-colors">
                                <td className="px-6 py-5">
                                  <p className="text-sm font-black text-text-main dark:text-white uppercase tracking-tight">{budget.category}</p>
                                  <p className="text-[10px] font-bold text-text-muted">Target: {formatAmount(budget.limit)}</p>
                                </td>
                                ...
                                <td className="px-6 py-5 text-right">
                                  <p className={`text-sm font-black ${isOver ? 'text-red-500' : 'text-emerald-500'}`}>
                                    {isOver ? '+' : '-'}{formatAmount(Math.abs(budget.limit - budget.spent))}
                                  </p>
                                  <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest">{budget.percentage.toFixed(0)}% Utilized</p>
                                </td>
                              </tr>
                            );
                          })}
                    ...
                    <div className="py-12 border-2 border-dashed border-border rounded-3xl text-center">
                      <p className="text-text-muted font-black text-[10px] uppercase tracking-widest opacity-40">No active budget allocations for this audit period</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="h-[400px] flex flex-col items-center justify-center text-center bg-card rounded-[2.5rem] border border-dashed border-border gap-6">
              ...
              <p className="text-text-muted font-bold px-12 text-sm leading-relaxed max-w-sm uppercase tracking-widest">
                Official statement unavailable for <span className="text-primary">{MONTHS[selectedMonth]} {selectedYear}</span> due to lack of recorded transactions.
              </p>
            </div>
          )}
        </div>

        {/* Formal Export Sidebar */}
        <div className="lg:col-span-4 space-y-6">
          <div className="p-8 bg-card rounded-[2.5rem] border border-border shadow-soft flex flex-col items-center text-center">
            ...
            <h3 className="text-base font-black text-text-main dark:text-white mb-2 uppercase tracking-tight">Export Statement</h3>
            ...
            <div className="w-full space-y-3">
              ...
              {exportError && (
                <div className="w-full p-4 bg-[#7F1D1D] rounded-2xl border border-red-500/30 text-left relative overflow-hidden transition-all mt-4" id="export-error-container">
                  <div className="flex gap-3">
                    <AlertCircle className="text-red-400 shrink-0 mt-0.5" size={16} />
                    <div className="flex-1">
                      <p className="text-xs font-black text-white uppercase tracking-wider mb-1">Export Failed</p>
          ...
          <div className="p-8 bg-primary text-white rounded-[2.5rem] shadow-xl shadow-primary/20 relative overflow-hidden group">
            <div className="relative z-10">
              <h4 className="text-sm font-black uppercase tracking-widest mb-4">Pro Insight</h4>
