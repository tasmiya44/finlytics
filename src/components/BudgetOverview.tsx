...
export default function BudgetOverview({ expenses, budgets, onOpenSettings }: { expenses: any[], budgets: any[], onOpenSettings: () => void }) {
  ...
  return (
    <div className="bg-card p-8 rounded-[2.5rem] border border-border shadow-polish mb-8">
      <div className="flex items-center justify-between mb-8 px-1">
        <div>
          <h3 className="text-xl font-extrabold text-text-main dark:text-white uppercase tracking-tight">{t('analytics.budget_tracking')}</h3>
          <p className="text-[11px] font-bold text-text-muted dark:text-slate-400 uppercase tracking-widest mt-0.5">{t('analytics.current_goals')}</p>
        </div>
        <button 
          onClick={onOpenSettings}
            ...
            >
              <div className="flex justify-between items-end px-1">
                <div className="flex items-center gap-2.5">
                  <div className={`p-2 rounded-lg ${isOver ? 'bg-danger/10 text-danger' : isWarning ? 'bg-warning/10 text-warning' : 'bg-primary/10 text-primary'}`}>
                    {isOver && <AlertCircle size={14} />}
                    {isWarning && <TrendingUp size={14} />}
                    {stat.percent < 80 && <CheckCircle2 size={14} />}
                  </div>
                  <span className="text-[14px] font-black text-text-main dark:text-white uppercase tracking-tight">{stat.category}</span>
                </div>
              ...
              <div className="h-2 w-full bg-border/30 dark:bg-slate-800 rounded-full overflow-hidden border border-border/50 shadow-inner">
                <motion.div 
                  ...
                  className={`h-full rounded-full transition-colors ${
                    isOver ? 'bg-danger shadow-[0_0_8px_rgba(239,68,68,0.4)]' : isWarning ? 'bg-warning shadow-[0_0_8px_rgba(245,158,11,0.4)]' : 'bg-primary shadow-[0_0_8px_rgba(99,102,241,0.4)]'
                  }`}
                />
              </div>
              ...
              <div className="flex justify-between items-center px-1">
                {isOver ? (
                  ...
                  <span className="text-[9px] font-black text-warning uppercase tracking-widest">Caution: {stat.percent.toFixed(0)}% Used</span>
                ) : (
                  <span className="text-[9px] font-black text-success uppercase tracking-widest">Good ({(100 - stat.percent).toFixed(1)}% Left)</span>
                )}
                <span className="text-[9px] font-black text-text-muted dark:text-slate-400 uppercase tracking-widest">{stat.percent.toFixed(0)}%</span>
              </div>
            </motion.div>
          );
        })}
