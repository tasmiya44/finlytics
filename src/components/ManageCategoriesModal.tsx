  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <motion.div
        ...
        className="bg-card w-full max-w-lg rounded-[2.5rem] border border-border shadow-2xl p-8 relative overflow-hidden flex flex-col max-h-[85vh]"
      >
        ...
        <div className="flex items-center justify-between mb-8 relative z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-black text-text-main tracking-tight">Manage Categories</h2>
              <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest leading-none mt-1">Configure Classifications</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="space-y-6 relative z-10 flex-1 overflow-y-auto pr-2 thin-scrollbar">
          ...
          <div className="space-y-3">
            {categories.map((cat) => {
              ...
              return (
                <div 
                  key={cat.id} 
                  className="flex items-center justify-between p-4 bg-bg dark:bg-slate-900/50 rounded-2xl border border-border/40 group hover:border-primary/20 transition-all"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-text-main">{cat.name}</p>
                      {isDefault && (
        ...
        <div className="mt-8 pt-6 border-t border-border/50 relative z-10">
          ...
          <button
            onClick={onClose}
            className="w-full mt-6 py-4 bg-text-main text-white dark:bg-white dark:text-text-main rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] transition-all hover:opacity-90 active:scale-[0.98]"
          >
            Close Manager
          </button>
        </div>
